...
// broke: waldo told operators about a tier it would then refuse.
func TestEveryPrintedTierParsesBack(t *testing.T) {
	for _, tier := range AllTiers {
		got, err := ParseTier(tier.String())
		if err != nil {
			t.Errorf("ParseTier(%q) rejects a tier waldo prints: %v", tier.String(), err)
			continue
		}
		if got != tier {
...
// A name waldo used to accept must explain itself rather than being reported as
