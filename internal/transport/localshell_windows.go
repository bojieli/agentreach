...
//
// A local:// target means "this machine is the target", and waldo's floor is a
...
//
// Setting WALDO_LOCAL_SHELL makes it a deliberate act by someone who knows what
// they are pointing waldo at. The ordinary Windows arrangement — waldo and the
// agent on Windows, driving a remote POSIX host — needs none of this.
func localShell() (string, error) {
	if explicit := os.Getenv("WALDO_LOCAL_SHELL"); explicit != "" {
		return explicit, nil
	}
	return "", fmt.Errorf(
		"a local:// target needs a POSIX shell, and Windows is not a supported target.\n" +
			"waldo on Windows is meant to drive a *remote* POSIX host: point it at\n" +
			"ssh://host/path instead. If you genuinely want this machine to be the target\n" +
			"and have Git for Windows or MSYS2, set WALDO_LOCAL_SHELL to its bash.exe —\n" +
