...
// in every process, so parallel reach processes all chose `.reach.tmp.1` and
...
func TestTempPathShape(t *testing.T) {
	for _, dir := range []string{"/srv/app", "/", "/a/b/c"} {
		...
		// Identifiable as reach's, so debris from an interrupted write can be
		// recognised rather than puzzled over.
		if !strings.HasPrefix(path.Base(p), ".reach.tmp.") {
			t.Errorf("TempPath(%q) = %q, which nothing would recognise as reach's", dir, p)
		}
		...
		// POSIX separators: these are the target's paths, whatever reach runs on.
