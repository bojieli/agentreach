...
//
//	Two reach processes writing to one directory at the same time must never
//	choose the same temporary name.
//
// *Processes*, not goroutines. reach runs one process per tool call and a
...
// process, so parallel writers all chose `.reach.tmp.1` and the exclusive
...
//
// The `.reach.tmp.` prefix is deliberate: anything left behind by an
// interrupted write is immediately identifiable as reach's, and `reach doctor`
...
func TempPath(dir string) string {
	...
	if _, err := rand.Read(r[:]); err != nil {
		...
		return path.Join(dir, fmt.Sprintf(".reach.tmp.%d", n))
	}
	return path.Join(dir, fmt.Sprintf(".reach.tmp.%s.%d", hex.EncodeToString(r[:]), n))
}
