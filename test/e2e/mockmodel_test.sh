...
# with waldo — so the mock gets its own tests.
set -uo pipefail
cd "$(dirname "$0")" && source ./lib.sh

PORT="${WALDO_MOCK_PORT:-8917}"
...
assert_contains "$models" 'waldo-mock' "the mock model is advertised"

stream="$(curl -s --max-time 8 -X POST "http://127.0.0.1:$PORT/v1/chat/completions" \
  -H 'Content-Type: application/json' \
  -d '{"model":"waldo-mock","stream":true,"messages":[{"role":"user","content":"hi"}]}')"
...
second="$(curl -s --max-time 8 -X POST "http://127.0.0.1:$PORT/v1/chat/completions" \
  -H 'Content-Type: application/json' \
  -d '{"model":"waldo-mock","stream":true,"messages":[
        {"role":"user","content":"hi"},
        {"role":"assistant","tool_calls":[{"id":"call_waldo_1","type":"function","function":{"name":"read","arguments":"{}"}}]},
        {"role":"tool","tool_call_id":"call_waldo_1","content":"REMOTE_CONTENT_MARKER"}]}')"
