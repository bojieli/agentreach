...
LARGE_MB="${WALDO_BENCH_LARGE_MB:-20}"
SMALL_READS="${WALDO_BENCH_SMALL_READS:-20}"
...
if [[ -n "${WALDO_BENCH_SSH_HOST:-}" ]]; then
  # ---------------------------------------------------- a host you already have
  HOST="$WALDO_BENCH_SSH_HOST"
  WS="${WALDO_BENCH_WORKSPACE:-/tmp}/waldo-bench-$$"
  LABEL="$HOST"
...
EOF
  ...
  for _ in $(seq 40); do
    ssh -F "$WALDO_SSH_CONFIG" "$HOST" true 2>/dev/null && break
    sleep 0.25
  done
  ssh -F "$WALDO_SSH_CONFIG" "$HOST" true 2>/dev/null \
...
for tier in posix sftp pipe agent; do
  ...
  t=$(now)
  "$ROOT/waldo" fs write "$WS/w.$tier" --session bench < "$BENCH_DIR/read.$tier"
  bigwrite=$(since "$t")
...
if [[ -z "${WALDO_BENCH_SSH_HOST:-}" ]]; then
  printf '\nOver loopback, so round trips are free. Re-run with WALDO_BENCH_SSH_HOST\n'
