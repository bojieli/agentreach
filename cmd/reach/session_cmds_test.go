...
// a newer reach, one naming a tier that no longer exists. Both of those
// failures suggest `reach down` as the way out, which made the advice a loop.
// The operator's only remaining move was deleting a file out of ~/.reach by
// hand.
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, err := os.Stat(p); !os.IsNotExist(err) {
				t.Errorf("%s still exists after `reach down`", p)
			}
		})
	}
}
...
// `reach status NAME` shows one session, which is what the help has always
...
// An unknown flag must not be swallowed. status has none, so `reach status
...
// not: `reach env --session prod` failed with "flag provided but not defined"
// while `reach log --session prod` worked, so the right spelling depended on
...
func TestSessionNameIsAcceptedTheSameWayEverywhere(t *testing.T) {
	...
	for name, run := range commands {
		for _, form := range [][]string{
			...
			// command having something to report: `reach log prod` legitimately
			...
			if err != nil && strings.Contains(err.Error(), "flag provided but not defined") {
				t.Errorf("reach %s %v: %v", name, form, err)
			}
...
// A bare `reach status` lists everything even inside a harness's shell, where
// $REACH_SESSION is set. Narrowing to one session there would hide the others
...
func TestStatusIgnoresTheAmbientSession(t *testing.T) {
	...
	for _, want := range []string{"one", "two"} {
		if !strings.Contains(out, want) {
			t.Errorf("status omitted %q with REACH_SESSION set:\n%s", want, out)
		}
	}
}
