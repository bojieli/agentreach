...
// reachGeminiSettingsMarker is written into settings.json so `reach doctor`
// can confirm the file was written by reach, not hand-edited by the user.
const reachGeminiSettingsMarker = "reach-managed"

// managedGeminiHome creates (or refreshes) the managed HOME directory reach
...
// reach sets HOME to this directory. The settings.json it contains excludes
...
// every launch keeps the file current when this reach binary changes what it
// excludes.
//
...
func writeManagedGeminiSettings(geminiDir string) error {
	settings := map[string]any{
		// Mark the file as reach-managed so doctor can identify it.
		"_reach": reachGeminiSettingsMarker,
