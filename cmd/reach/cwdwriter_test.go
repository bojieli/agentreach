...
func TestCwdWriterExtractsMarker(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			w := &cwdCapturingWriter{out: &buf, marker: "__reach_cwd__"}
...
func TestCwdWriterNeverLeaksMarker(t *testing.T) {
	var buf bytes.Buffer
	w := &cwdCapturingWriter{out: &buf, marker: "__reach_cwd__"}
	_, _ = w.Write([]byte("compiling...__reach_cwd__/build\n"))
	_ = w.Captured()
	if bytes.Contains(buf.Bytes(), []byte("__reach_cwd__")) {
		t.Errorf("reach bookkeeping leaked into the agent's stderr: %q", buf.String())
	}
}
