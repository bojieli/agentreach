...
// reach as a remote environment. It speaks codex's exec-server JSON-RPC
// protocol on stdin/stdout, backing every method with the session named by
// REACH_SESSION (or --session). Stderr is free for logging; stdout carries
...
func cmdExecServer(ctx context.Context, args []string) int {
	...
	if err != nil {
		...
		fmt.Fprintln(os.Stderr, "reach exec-server:", err)
		return 1
	}
