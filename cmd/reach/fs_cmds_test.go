...
// called Search throughout reach's own code — answered "flag provided but not
...
func TestFSAliasesNameTheRightSubcommand(t *testing.T) {
	for alias, want := range fsAliases {
		...
		if !strings.Contains(err.Error(), "reach fs "+want) {
...
// exactly that until `reach fs mv` existed.
...
// With the check after parseFlags, reach answered "flag provided but not
