...
// shimHome sets REACH_HOME to a directory removed on a best-effort basis.
//
// t.TempDir fails a test whose cleanup cannot remove everything, and on Windows
// it cannot: installProgramAlias hard-links reach's own binary to make the
...
// Windows and about a test executing the thing it is testing, not about reach,
...
func shimHome(t *testing.T) string {
	t.Helper()
	home, err := os.MkdirTemp("", "reach-home-")
	if err != nil {
		t.Fatal(err)
	}
...
// TestShimAliasIsInstalledAndRecognised covers the whole shim round trip: reach
...
func TestShimAliasIsInstalledAndRecognised(t *testing.T) {
	...
	if got := programBase(alias); got != shimName {
		t.Errorf("programBase(%q) = %q, want %q — reach would not recognise its own shim", alias, got, shimName)
	}
...
// upgrade. A stale shim is an old reach running inside a tool call, which
// surfaces as the harness misbehaving rather than as a reach problem.
func TestShimAliasIsRefreshedWhenStale(t *testing.T) {
	...
	// Replace it with something that is definitely not reach.
	...
	if programAliasIsCurrent(alias, self) {
		t.Error("a stale alias was reported as current; an upgrade would leave the old reach in the tool path")
	}
...
// one. The harness then launches without reach's shim directory in front, finds
...
func TestPrependPathFindsTheRealPathVariable(t *testing.T) {
	...
	for _, spelling := range spellings {
		t.Run(spelling, func(t *testing.T) {
			...
			got := prependPath(env, "/reach/shim")
			...
			want := "/reach/shim" + string(filepath.ListSeparator) + "/usr/bin"
...
// case leaves Windows to choose between them, which is not a decision reach
...
func TestSameDirIsCaseInsensitiveOnWindows(t *testing.T) {
	...
	if !sameDir(`C:\Users\Test\.reach\shim`, `c:\users\test\.reach\SHIM`) {
...
// whether reach believes a harness is installed. Windows has no execute bit —
// os.Stat reports 0444 or 0666 and nothing else — so a mode test there is a
// test that always fails, and reach would report every harness as missing.
func TestIsExecutableFileMatchesPlatformRules(t *testing.T) {
	...
	if !isExecutableFile(runnable) {
		t.Errorf("%s was not reported as executable; reach would say the harness is not installed", runnable)
	}
...
func TestProgramBaseStripsTheWindowsExtension(t *testing.T) {
	...
	for _, tc := range []struct{ argv0, want string }{
		{`C:\reach\shim\bash.exe`, "bash"},
		{`C:\reach\shim\BASH.EXE`, "BASH"},
		{`C:\reach\bin\reach-shell-prefix.exe`, shimName},
	} {
