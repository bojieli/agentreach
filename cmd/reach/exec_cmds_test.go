...
func localSession(t *testing.T, workspace string) *session.Session {
	...
	// to assert on this platform. What reach does *on* Windows, driving a remote
	...
	s := &session.Session{
		...
		Created: time.Now(), Tier: reach.TierPOSIX, Timeout: 30 * time.Second,
	}
...
// Claude Code tracks the working directory by reading a file reach is told to
...
// — or replacing it with reach's own — turns a failed build into a successful
