...
// reachGooseConfigMarker marks reach's managed extensions section so that
// repeated launches do not append the restriction twice.
const reachGooseConfigMarker = "# reach: developer extension restricted to shell tool"

// managedGooseHome builds (or refreshes) the GOOSE_PATH_ROOT reach launches
...
// extensions section with a reach-controlled version that restricts the
...
func managedGooseHome(sessName string) (string, error) {
	dir, err := reachSubdir(filepath.Join("goose-home", sessName))
	if err != nil {
		return "", err
	}
...
func managedGooseConfig(baseYAML string) string {
	if strings.Contains(baseYAML, reachGooseConfigMarker) {
		// Already a reach-managed config; return unchanged to avoid a second
	...
	// Append reach's controlled extensions section.
	out.WriteString(reachGooseConfigMarker + "\n")
