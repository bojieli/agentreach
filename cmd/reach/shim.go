...
// shimName is the logical name reach answers to when acting as a harness shell
// shim.
//
...
// "reach shell-prefix" is looked up as a single filename and fails. Dispatching
...
const shimName = "reach-shell-prefix"

// isShimInvocation reports whether this process was started through the shim
// alias rather than as `reach <command>`.
func isShimInvocation() bool {
	return programBase(os.Args[0]) == shimName
}

// binDir is where reach keeps the shim alias.
func binDir() (string, error) {
	return reachSubdir("bin")
}

// reachSubdir resolves a directory beneath reach's state directory, creating it.
func reachSubdir(name string) (string, error) {
...
//
// It is checked on every use rather than created once: reach may have been
...
// where the harness reports it as a broken shell rather than as a reach problem.
