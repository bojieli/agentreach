...
const logUsage = `waldo log — what waldo did on the target

  waldo log [session] [--limit N] [--json] [--failed] [--path]

waldo records every command it runs on a target and every file it changes
there. This prints that record.

The point of it is the situation waldo is built for: you pointed an agent at a
	...
	if err != nil {
		if os.IsNotExist(err) {
			return fmt.Errorf("no audit log for session %q yet.\n"+
				"waldo writes one as soon as it runs something on the target", name)
		}
		return err
	}
