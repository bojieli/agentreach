...
// `waldo up ssh://host/path --name build` silently ignored --name and made a
...
// Everything after `--` belongs to the target, not to waldo. Without this,
// `waldo exec -- ls -la` would hand -la to waldo's own flag parser and fail on
...
func TestParseFlagsTreatsEverythingAfterDashDashAsPositional(t *testing.T) {
	for _, tc := range []struct {
		...
		{"target flags are not waldo's", []string{"--", "ls", "-la"}, "default", []string{"ls", "-la"}},
		{"waldo flags before the separator", []string{"--name", "s", "--", "ls", "-la"}, "s", []string{"ls", "-la"}},
		{"a waldo flag name after it belongs to the target",
			[]string{"--", "echo", "--name", "not-waldos"}, "default", []string{"echo", "--name", "not-waldos"}},
