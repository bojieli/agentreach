...
// on one machine; a separate client process talks to it over TCP. waldo
...
func cmdCrush(ctx context.Context, args []string) int {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "waldo: crush is not installed or not in PATH")
		return 1
	}
	...
	if !ok {
		if *force {
			fmt.Fprintln(os.Stderr,
				"waldo: WARNING: --force bypasses server mode. crush will run entirely\n"+
					"waldo: on the LOCAL machine; its tools act on the operator's filesystem,\n"+
					"waldo: not on the target.")
			env := append(os.Environ(), "WALDO_SESSION="+sessName)
			argv := append([]string{binPath}, pos...)
			return replaceProcess(ctx, binPath, argv, env)
		}
		fmt.Fprintln(os.Stderr, "waldo: crush server mode requires an SSH session (ssh://).")
		fmt.Fprintln(os.Stderr, "waldo: Docker and local sessions are not supported.")
		fmt.Fprintln(os.Stderr, "waldo: Use --force to launch crush locally (tools will act on the local machine).")
		return 1
	}
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "waldo: cannot find a free local port:", err)
		return 1
	}

	serverCmd := fmt.Sprintf("crush server --host tcp://127.0.0.1:%d", port)
	fmt.Fprintf(os.Stderr,
		"waldo: starting crush server on %s (port %d)...\n",
		s.Target.Describe(), port)

	// Start crush server on the target. Open keeps the SSH channel alive for
	// the server's lifetime. When this waldo process is exec-replaced by the
	...
	if _, err := sshT.Open(ctx, serverCmd); err != nil {
		fmt.Fprintln(os.Stderr, "waldo: cannot start crush server on the target:", err)
		return 1
	}

	// Add a local port forward to the existing ControlMaster connection. This
	// requires Multiplex=true, which waldo sets when the session was created
	...
	if err := sshT.ForwardLocal(ctx, port, port); err != nil {
		fmt.Fprintln(os.Stderr, "waldo: cannot set up the SSH port forward:", err)
		fmt.Fprintln(os.Stderr, "waldo: ensure the session was created with a multiplexing-capable ssh client.")
		return 1
	}
	...
	if err := waitForCrushPort(ctx, port); err != nil {
		fmt.Fprintln(os.Stderr, "waldo: crush server did not come up in time:", err)
		return 1
	}

	hostURL := fmt.Sprintf("tcp://127.0.0.1:%d", port)
	fmt.Fprintf(os.Stderr,
		"waldo: Crush -> %s (server mode; every tool acts on the target)\n",
		s.Target.Describe())
