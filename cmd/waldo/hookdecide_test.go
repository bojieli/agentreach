...
func (f *fakeOps) Read(_ context.Context, p string, off, n int64) ([]byte, error) {
	...
	data, ok := f.files[p]
	if !ok {
		return nil, &waldo.NotFoundError{Path: p}
	}
...
func TestHookRoutePassesThroughUnrelatedTools(t *testing.T) {
	...
	// to a tool waldo knows nothing about would point it at a mirror directory
	...
	for _, tc := range []struct {
		...
		if ok {
			t.Errorf("%s was routed to the mirror; waldo only rewrites tools it knows", tc.tool)
		}
		if reply.HookSpecificOutput != nil {
			t.Errorf("%s got a decision (%+v); waldo must not interfere", tc.tool, reply.HookSpecificOutput)
		}
	}
}
...
func TestHookRouteIgnoresMalformedInput(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// blocking the tool over it would break a session waldo could have
...
// harness's settings, a scratch note. Rewriting those would make waldo reach
...
// An event name waldo does not know must be inert. Harnesses add hook events
...
// still mentions waldo after the session is gone.
func TestHookDecideWithoutASession(t *testing.T) {
	t.Setenv("WALDO_HOME", t.TempDir())
	t.Setenv("WALDO_SESSION", "does-not-exist")
