// Command reach-helper is the optional helper reach installs on a target for
// the helper tier.
//
// It is the only thing reach ever writes to a target's disk, and only when the
...
//
// The protocol is byte-identical to the tier-2 Python handler's, so reach's
...
// target, so an upgraded reach installs a new helper instead of silently reusing
// a stale one.
var version = "dev"
...
func main() {
	if len(os.Args) > 1 {
		switch os.Args[1] {
		case "--selftest":
			// reach runs this after uploading and compares both fields against
			...
			// target, and reach reinstalls rather than trusting it.
			sum, err := selfDigest()
			if err != nil {
				fmt.Fprintln(os.Stderr, "reach-helper: cannot hash self:", err)
				os.Exit(1)
			}
			fmt.Printf("reach-helper %s %s %s/%s\n", version, sum, runtime.GOOS, runtime.GOARCH)
			return
		case "--version":
			fmt.Printf("reach-helper %s %s/%s\n", version, runtime.GOOS, runtime.GOARCH)
			return
		case "serve":
			// the default; accepted so the command line reads clearly
		default:
			fmt.Fprintf(os.Stderr, "reach-helper: unknown argument %q\n", os.Args[1])
			os.Exit(2)
		}
	}
	if err := serve(os.Stdin, os.Stdout); err != nil && !errors.Is(err, io.EOF) {
		fmt.Fprintln(os.Stderr, "reach-helper:", err)
		os.Exit(1)
	}
}
...
func opWrite(req request, payload []byte) (int, error) {
	...
	tmp := filepath.Join(dir, fmt.Sprintf(".reach.tmp.%d.%d", os.Getpid(), req.ID))
