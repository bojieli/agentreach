...
waldo's premise is that **the target is not trusted**. It exists precisely for
...
this in waldo; use plain `ssh` if you have a reason to.

**Local paths are stripped where waldo can do it safely.** Claude Code's
...
benefit, so waldo removes it. waldo does *not* attempt to rewrite arbitrary
commands: a false positive that silently mangled a command would be worse than
the leak it prevented.

**Connections do not outlive sessions.** `waldo down` tears down the
...
## What waldo does not protect you from

**Prompt injection from target output.** This is the sharpest risk in this
design, and it is not fully solvable at waldo's layer.
...
Nothing about running the agent locally prevents this; if anything, waldo makes
it more relevant, because the whole point is pointing an agent at machines you
do not control.

Mitigations, in order of effectiveness:

- Keep secrets out of the agent's local environment. waldo does not need them
  and neither should the session.
- Prefer a session per target. waldo binds a session to exactly one target, so
...
**A malicious target lying about its own state.** waldo reports what the target
says. A compromised host can return whatever it likes for `hostname`, file
contents or exit statuses. waldo verifies transport integrity, not target
honesty.
...
That also means waldo inherits whatever your config does, including any
`ForwardAgent yes` you have set for a host in `~/.ssh/config`. waldo passes
...
In `exec` mode with Claude Code, waldo denies `Read`, `Edit`, `Write`, `Glob`
...
the waldo version, the harness and version, and the target's `uname -sm`.
