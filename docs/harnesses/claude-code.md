...
envelope as a **single argv element**. reach installs a symlink named
`reach-shell-prefix` and points the variable at it, because Claude Code stats
the value as a program path: a value like `"reach shell-prefix"` is looked up as
...
**mirror** — additionally wires reach's hook into the file tools. Each file is
...
- Output is streamed, but a command producing enormous output is capped; reach
